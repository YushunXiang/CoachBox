    ...
    , ui(new Ui::Window)
{
    ...
    if(DB_init()!=0)qDebug()<<"init rtdb error!";
...
void Window::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    {
        if(info_page->isShow)emit info_page->exit();
        else if(settings_page->isShow)emit settings_page->exit();
    }
}
...
void Window::timeoutProcess()
{
    if(isComm)
    {
        ...
        if(DB_put(MESSAGEFROMCOACHINFO, &coach_info)==-1)qDebug()<<"DB_put error!";
    }
}
void Window::updateCoachinfo()
{
    ...
    printLog("调试指令更新:Head=0 MatchMode="+QString().setNum(coach_info.MatchMode)+" MatchType=0");
}
...
void Window::ConnectToRefereeBox()
{
    ...
    if(isConnectRB)
    {
        ...
        printLog("裁判盒连接成功");
        if(RBClient->isReadable())printGameinfo("开始接受裁判指令");
        else printGameinfo("连接不可用");
    }
}
